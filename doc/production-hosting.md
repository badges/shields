...
- The server runs SSH. @espadrine has access.
- Deploys are made using a git post-receive hook. @paulmelnikow and @espadrine
  have access.
...
1.  The GitHub tokens we collect are saved on each server in JSON files on disk.
    ...
    @paulmelnikow and @espadrine have access. The GitHub OAuth application is
    part of @espadrine's account but
    [it should be transferred to the badges org][oauth transfer].
...
I'm not sure where the DNS is registered. @olivierlacan has access.

## Logs

Logs are available on the individual servers via SSH. @espadrine has access.
...
1.  Too many bottlenecks! ([#2577][issue 2577])
2.  The only way to inspect the commit on the server is with `git ls-remote`.
3.  The production deploy installs `devDependencies`. It does not honor
