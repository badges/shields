...
describe('Prometheus metrics route', function() {
  ...
  it('returns metrics for request with allowed "x-forwarded-for" header value', async function() {
    startServer({
      prometheus: { enabled: true, allowedIps: /^127\.0\.0\.200$/ },
    })
    const headers = { 'x-forwarded-for': '127.0.0.200' }
