  ...
  describe('css color regex', function () {
    it('should check if given valid css3 colors', function () {
      ...
      colorsFalse.forEach((color) => {
        expect(JSON.parse(makeBadge({ text: ['name', 'Bob'], colorB: color, format: 'json', template: '_shields_test' })).colorB, `${color} should return #e05d44`).to.equal('#e05d44');
      });
    });
    ...
    it('should check if given valid colorscheme colors', function () {
      ...
      colorsFalse.forEach((color) => {
        expect(JSON.parse(makeBadge({ text: ['name', 'Bob'], colorscheme: color, format: 'json', template: '_shields_test' })).colorB, `${color} should return #e05d44`).to.equal('#e05d44');
      });
    });
  });
