...
const globalQueryParams = new BetterSet([
  'label',
  'style',
  'link',
  'logo',
...
// Incorporate the provided query params into the global query params and
// return a new, sorted array.
function flattenQueryParams(queryParams) {
  const union = new BetterSet(globalQueryParams);
  union.addAll(queryParams)
  return Array.from(union).sort();
}

// handlerOptions can contain:
// - handler: The service's request handler function
// - queryParams: An array of custom query field names the service uses
//
...
// parameter not working at all (undesirable, but easy to debug) rather than
// indeterminate behavior that depends on the cache state (undesirable, but
// hard to debug).
//
...
function handleRequest (handlerOptions) {
  ...
  return (queryParams, match, end, ask) => {
    ...
    const maxAge = queryParams.maxAge;
    if (maxAge !== undefined && /^[0-9]+$/.test(maxAge)) {
    ...
    // Use sindresorhus query-string instead of the builtin module because it
    // sorts the keys.
