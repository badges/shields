...
function versionReduction(versions) {
  ...
  // Do not forget to update this list!
  const phpReleases = [
      '5.0', '5.1', '5.2', '5.3', '5.4', '5.5', '5.6', // 5.x
      '7.0', '7.1', '7.2', // 7.x
  ];

  // remove duplicates
  versions = versions.filter((value, index, self) => self.indexOf(value) === index);
  versions.sort();
  // versions intersect
  versions = versions.filter((n) => phpReleases.includes(n));
  ...
  if (first + versions.length - 1 === last) {
    if (last === phpReleases.length - 1) {
      return '>= ' + (versions[0][2] === '0' ? versions[0][0] : versions[0]);
    }
