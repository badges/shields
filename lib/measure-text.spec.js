...
describe.only('PDFKitTextMeasurer with DejaVu Sans', function () {
  let measurer;
  beforeEach(function () {
...
function registerTests(fontPath, displayName, skip) {
  ...
  describe.only(`QuickTextMeasurer with ${displayName}`, function () {
    let quickMeasurer;
    if (! skip) {
      before(function () {
    ...
    context.only('when given non-ASCII strings', function () {
      const strings = [
        starRating(3.5),
        '\u2026',
      ];
