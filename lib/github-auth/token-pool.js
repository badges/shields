...
class TokenPool {
  ...
  serializeDebugInfo({ sanitize = true } = {}) {
    ...
    function sanitizeToken({ id, usesRemaining, nextReset, isValid, isFrozen }) {
      return {
        id: sha(id),
        data: '[redacted]',
        usesRemaining,
    ...
    const utcEpochSeconds = this.constructor.utcEpochSeconds();
    const allValidTokenIds = this.allValidTokenIds();
    const fifoQueue = this.fifoQueue.map(t => t.getDebugInfo());
    const priorityQueue = [];
    this.priorityQueue.forEach(t => prorityQueue.push(t.getDebugInfo()));

    if (sanitize) {
      return {
        utcEpochSeconds,
        allValidTokenIds: allValidTokenIds.map(sanitizeToken),
        fifoQueue: fifoQueue.map(sanitizeToken),
        sanitized: true,
      };
    } else {
      return {
