...
//   npm run test:services -- --skip-intercepted
//
// Run tests on a given instance:
//  npm run test:services -- --url=https://test.shields.io --skip-intercepted
//
...
const testedServerUrl = args.url && new URL(args.url)
const skipIntercepted = args['skip-intercepted']
const runner = new Runner({ testedServerUrl, skipIntercepted })
runner.prepare()
...
if (testedServerUrl) {
  console.log(`Service tests will check ${testedServerUrl}.`)
}
if (skipIntercepted) {
  console.log('Service tests which intercepts requests will be skipped.')
}
