...
// testing hex, rgb, rgba, css word colors (red, blue, maroon etc.)
// Optimized hex color regex - #1602
const cssColor = /^(#([\da-f]{3}){1,2}|rgba?\(([01]?\d?\d|2[0-4]\d|25[0-5]),([01]?\d?\d|2[0-4]\d|25[0-5]),([01]?\d?\d|2[0-4]\d|25[0-5])(,(1|0(\.\d+)?))?\)|[a-z]{3,20})$/i; 
...
function makeBadge (measurer, {
  ...
  colorA = colorA || colorscheme,
  colorB = colorB || colorscheme,
  colorA = definedColorschemes[colorA] !== undefined ? definedColorschemes[colorA].colorA || undefined : cssColor.test(colorA) ? colorA : undefined,
  colorB = definedColorschemes[colorB] !== undefined ? definedColorschemes[colorB].colorB || undefined : cssColor.test(colorB) ? colorB : undefined;
