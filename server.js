  ...
  cache(function (data, match, sendBadge, request) {
    ...
    var modes = {
      ...
      'v': {
        ...
        version: true,
        process: function (data, badgeData) {
          var version = data.version ? data.version.num : data.crate.max_version
  ...
  cache(function (data, match, sendBadge, request) {
    ...
    request(apiUrl, function dealWithData (err, res, buffer) {
      ...
      try {
        var data = JSON.parse(buffer)
        if (data.receiving) {
          var money = parseInt(data.receiving, 10)
  ...
  cache(function (data, match, sendBadge, request) {
    ...
    request(apiUrl, function (err, res) {
      ...
      try {
        ...
        var percentage = parseInt(score, 10)
  ...
  cache(function (data, match, sendBadge, request) {
    ...
    request(apiUrl, {}, function (err, res, buffer) {
      ...
      try {
        ...
        // Which branch are we dealing with?
        if (branch === null) { branch = data.default_branch }
        var response = data.applications[branch].build_status.status
        badgeData.text[1] = res
        if (response === 'passed') {
          badgeData.colorscheme = 'brightgreen'
        } else if (response === 'failed' || response === 'error') {
          badgeData.colorscheme = 'red'
...
function latestVersion (versions) {
  ...
  var semver_versions = versions.map(function (version) {
